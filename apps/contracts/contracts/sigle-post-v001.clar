...
(define-public (set-mint-enabled
    (enabled bool)
  )
  (begin
...
(define-public (set-base-token-uri
    (new-uri (string-ascii 210))
  )
  (begin
    (asserts! (is-eq tx-sender (var-get contract-owner)) (err ERR-NOT-AUTHORIZED))
...
(define-public (accept-ownership)
  (begin
    (asserts!
      (is-eq tx-sender
        (unwrap! (var-get pending-contract-owner) (err ERR-NOT-AUTHORIZED))
      )
      (err ERR-NOT-AUTHORIZED)
    )
    (var-set contract-owner
      (unwrap! (var-get pending-contract-owner) (err ERR-NOT-AUTHORIZED))
    )
...
(define-public (mint
  ...
  (let (
    (next-token-id (+ u1 (var-get last-token-id)))
...
(define-private (is-sender-owner
  ...
  (let (
    (owner (unwrap! (nft-get-owner? sigle-post id) false))
  )
...
(define-read-only (get-listing-in-ustx
    (id uint)
  )
  (map-get? market id)
)
    ...
    (listing
      {
        ...
        royalty: (var-get royalty-percent),
      })
  )
...
(define-public (buy-in-ustx
  ...
  (let (
    (owner (unwrap! (nft-get-owner? sigle-post id) (err ERR-NOT-FOUND)))
...
(define-public (set-royalty-percent
    (royalty uint)
  )
  (begin
    (asserts! (is-eq tx-sender (var-get contract-owner)) (err ERR-NOT-AUTHORIZED))
    (asserts! (and (>= royalty u0) (<= royalty u1000))
      (err ERR-INVALID-PERCENTAGE)
    )
    (print {
      a: "set-royalty-percent",
      royalty: royalty,
    })
