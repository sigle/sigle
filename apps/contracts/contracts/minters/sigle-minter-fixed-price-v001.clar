...
(define-public (init-mint-details
  ...
  (begin
    ...
    (ok
      (map-set contract-mint-config tx-sender
        {
          price: price,
          start-block: start-block,
          end-block: end-block,
...
(define-public (set-mint-details
  ...
  (let (
    (mint-config
      (unwrap! (map-get? contract-mint-config (contract-of token-contract))
    ...
    (ok
      (map-set contract-mint-config (contract-of token-contract)
        {
          price: price,
          start-block: start-block,
          end-block: end-block,
...
(define-public (mint
    (token-contract sigle-post-trait)
    (quantity uint)
    (mintReferrer (optional principal))
  ...
  (let (
    ...
    (try!
      (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
        transfer referrer-fee tx-sender referrer-address none
      ))
    (try!
      (if (<= u1 quantity)
